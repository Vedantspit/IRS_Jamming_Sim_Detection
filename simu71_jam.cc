...
int main(int argc, char *argv[])
{
    ...
    // Don't attach jammer, let it act like a rogue transmitter
    mmwaveHelper->AttachToClosestEnb(jammerDevs, enbDevs);
    ...
    for (uint32_t i = 0; i < numUsers; ++i)
    {
        ...
        udpClient.SetAttribute("MaxPackets", UintegerValue(1000000000)); // 1e9
    ...
    // Set high TX power for jammer (unassociated)
    ...
    jammerPhy->SetTxPower(3000.0); // Increase jammer power
    ...
    // Enable SINR tracing (optional)
    // Config::ConnectWithoutContext("/NodeList/*/DeviceList/*/MmWaveUePhy/Sinr", MakeCallback(&PrintSinr));
